...
contract NFTMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        validListing(_nftAddress, _tokenId, _owner)
    {
        ...
        (address minter, uint256 royaltyFee) = NFTWallet(_nftAddress).royaltyInfo(_tokenId, price.sub(feeAmount));
        if (royaltyFee > 0) {
    ...
    function createOffer(
        ...
        INFTAuction auction = INFTAuction(addressRegistry.auction());
    ...
    function _isNFT(address _nftAddress) internal view returns (bool) {
        return
            INFTFactory(addressRegistry.nftFactory()).exists(_nftAddress);
    }
