    ...
    Ownable
{
    ...
    function mint(address _to, string calldata _cid, uint96 _royalty, string memory _name) public payable virtual returns (uint256) {
        require(hasRole(MINTER_ROLE, _msgSender()), "NFTWallet: must have minter role to mint");
        require(msg.value >= mintFee, "NFTWallet: insufficient funds to mint");

        uint256 newTokenId = tokenIdTracker.current();
        tokenIdTracker.increment();
        
        (bool success,) = feeRecipient.call{value : msg.value}("");
        require(success, "NFTWallet: transfer failed");
    ...
    function pause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "NFTWallet: must have pauser role to pause");
        _pause();
    }

    function unpause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "NFTWallet: must have pauser role to unpause");
        _unpause();
    }
